...
static void update_explored_cells(DroneSwarm* env, float* out_new_cells) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (cell_y >= grid_w) cell_y = grid_w - 1;
        int idx = cell_y * grid_w + cell_x;
        if (idx < 0) idx = 0;
        if (idx >= MAX_GRID * MAX_GRID) idx = MAX_GRID * MAX_GRID - 1;
...
static void compute_observations(DroneSwarm* env) {
    ...
    float base_x = env->cfg.base_pos[0];
    float base_y = env->cfg.base_pos[1];
    float r_sense_sq = env->cfg.r_sense * env->cfg.r_sense;
    float r_comm = env->cfg.r_comm;
    float r_comm_sq = r_comm * r_comm;
    ...
    for (int d = 0; d < n; d++) {
        ...
        if (k > 0) {
            ...
            for (int vid = 0; vid < v; vid++) {
                ...
                if (dist_sq < best_dist[k - 1]) {
                    ...
                    while (insert > 0 && dist_sq < best_dist[insert - 1]) {
                        ...
                        best_idx[insert] = best_idx[insert - 1];
                        insert--;
                    }
            ...
            for (int i = 0; i < k; i++) {
                ...
                float rel_x = (env->victim_pos[vid][0] - px) * inv_L;
                float rel_y = (env->victim_pos[vid][1] - py) * inv_L;
                out[10 + i * 3] = rel_x;
                out[10 + i * 3 + 1] = rel_y;
                out[10 + i * 3 + 2] = (env->victim_status[vid] == 1) ? 1.0f : 0.0f;
            }
        }
    }
}
...
void drone_swarm_init(DroneSwarm* env, const DroneSwarmConfig* cfg) {
    ...
    if (grid_w > MAX_GRID) grid_w = MAX_GRID;
    env->grid_w = grid_w;
}
